...
export const firebaseService = {
  ...
  // Get banner URL dynamically from Firebase
  async getBannerUrl() {
    try {
      const snapshot = await get(ref(database, "config/bannerUrl"))
      return snapshot.val() || "https://mining-master.onrender.com//assets/banner-BH8QO14f.png"
    } catch (error) {
      console.error("Error getting banner URL:", error)
      return "https://mining-master.onrender.com//assets/banner-BH8QO14f.png"
    }
  },
  ...
  async createConversion(userId: string, conversionData: any) {
    try {
      const conversionsRef = ref(database, `history/${userId}`)
  ...
  async getUserConversions(userId: string) {
    try {
      const snapshot = await get(ref(database, `history/${userId}`))
      const conversions = snapshot.val() || {}
      
      return Object.values(conversions).map((conversion: any) => ({
        ...conversion,
        packageImage: conversion.packageImage || null,
      }))
    } catch (error) {
